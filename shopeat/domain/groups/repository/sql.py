...
from shopeat.core.database import DBModel, Database
...
async def get_group_by_uid(session: AsyncSession, group_uid: str):
    ...
    query_result = await session.execute(select(Group).where(Group.uid == group_uid).options(selectinload(Group.members)))
    try:
...
async def get_group_member(session: AsyncSession, group: Group, account_uid: str):
    ...
    query_result = await session.execute(select(GroupMember).where(GroupMember.group_id == group.id, GroupMember.account_uid == account_uid))
    try:
        return query_result.scalar_one()
    except sqlalchemy.exc.NoResultFound:
        raise HTTPException(404, detail=f"Account {account_uid} is not member of group {group.uid}.")
...
class SQLGroupRepository(GroupRepository):
    ...
    async def get_several_by_member(self, account_uid: str) -> List[GroupReadDTO]:
        ...
        async with self.database.session.begin() as session:
            query = await session.execute(select(Group).join(GroupMember).where(GroupMember.account_uid == account_uid))
            groups = query.scalars().all()
            return [GroupReadDTO.from_orm(group) for group in groups]

    async def create(self, create_group_dto: GroupWriteDTO, group_owner: str) -> GroupReadDTO:
        async with self.database.session.begin() as session:
            group_data = create_group_dto.dict()
            group_data.update(uid=uuid4().hex)
            group = Group(**group_data)
            group.members.append(GroupMember(account_uid=group_owner, group_id=group.id, role="owner"))
            session.add(group)
            data = GroupReadDTO.from_orm(group)
            await session.commit()
            return data

    async def add_member(self, group_uid: str, add_member_dto: AddMemberDTO) -> GroupMemberDTO:
        async with self.database.session.begin() as session:
    ...
    async def remove_member(self, group_uid: str, account_uid: str) -> GroupMemberDTO:
        async with self.database.session.begin() as session:
            ...
            if len(group.members) == 1:
                raise HTTPException(403, detail="You can't leave group as you are the latest member of the group. Please delete the group instead.")

            number_of_owners = sum(1 for member in group.members if member.role == "owner")
            if number_of_owners == 1:
                raise HTTPException(403, detail="You need to define at least one new owner before leaving this group.")
            
            group_member = await get_group_member(session, group=group, account_uid=account_uid)
